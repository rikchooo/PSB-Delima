...
import { HiAcademicCap, HiBookOpen, HiDocumentText, HiExclamation, HiArrowLeft, HiHome } from 'react-icons/hi';
...
export default function HasilPengujianPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const nilaiRes = await apiFetch(`/api/pengujian/santri/${pendaftaranId}`);

        if (nilaiRes.ok) {
          const data = await nilaiRes.json();
          setNilaiData(data.data || null);
          setSantriData(data.data?.pendaftaran || null);
        } else if (nilaiRes.status !== 404) {
          const err = await nilaiRes.json();
  ...
  useEffect(() => {
    const assignRoom = async () => {
      if (!nilaiData || !santriData || assigningRoom || assignedRoom || nilaiData.pendaftaran?.room || nilaiData.pendaftaran?.room_id) return;

      const gender = santriData.jenis_kelamin === 'Perempuan' ? 'female' : 'male';
      setAssigningRoom(true);

      try {
        const roomsRes = await apiFetch(`/api/pendaftaran/studentrooms/available/${gender}`);
        if (roomsRes.ok) {
          ...
          if (availableRooms.length > 0) {
            ...
            const assignRes = await apiFetch(`/api/pendaftaran/studentrooms/${selectedRoom.id}/assign`, {
              method: 'POST',
            ...
            if (assignRes.ok) {
              ...
              setAssignedRoom(assignData.data || selectedRoom);
            }
          }
        }
      } catch (err) {
    ...
    if (nilaiData && santriData && !assignedRoom && !assigningRoom) {
      assignRoom();
    }
  }, [nilaiData, santriData, pendaftaranId, assigningRoom, assignedRoom]);
  ...
  // Helper component untuk baris detail nilai
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-12">
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Room Assignment */}
        {(assignedRoom || nilaiData.pendaftaran?.room) && (
          <div className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden mb-6">
            <div className="px-5 py-3.5 border-b border-slate-100 bg-indigo-50/50 flex items-center gap-2">
              ...
              <h3 className="font-semibold text-sm text-indigo-900 uppercase tracking-wide">Asrama</h3>
            </div>
            <div className="p-5">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-slate-900">
                     Kamar {(assignedRoom || nilaiData.pendaftaran.room).name}
                  </p>
                  <p className="text-xs text-slate-500 mt-1">
                     {(assignedRoom || nilaiData.pendaftaran.room).gender === 'male' ? 'Laki-laki' : 'Perempuan'} | Kuota: {(assignedRoom || nilaiData.pendaftaran.room).current_count}/{(assignedRoom || nilaiData.pendaftaran.room).quota}
                  </p>
                   {(assignedRoom || nilaiData.pendaftaran.room).ketua_kamar && (
                    <p className="text-xs text-slate-500 mt-1">
                      Ketua Kamar: <span className="font-medium text-slate-700">{(assignedRoom || nilaiData.pendaftaran.room).ketua_kamar}</span>
                    </p>
                  )}
                </div>
              </div>
